...
//   - top ~18% / bottom ~40% (headers + native chat)
//   - left/right ~12% on Facebook (side crop + reaction column)
// Keep ALL KiDi+ overlays inside the inner safe rectangle.
...
export function BroadcastComposition({
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-black text-white">
      ...
      {/* ── Inner safe rectangle (social crop + FB reaction column) ── */}
      <div
        className="pointer-events-none absolute z-30 flex flex-col justify-between"
        style={{
          top: SAFE_TOP,
          bottom: SAFE_BOTTOM,
          left: SAFE_X,
      ...
      >
        {/* Row 1: brand (left) + compact featured (inset from right) */}
        <div className="flex items-start justify-between gap-3">
          <div
            className="min-w-0 max-w-[55%] rounded-2xl px-2.5 py-1.5"
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span className="truncate text-[12px] font-bold">{hostName}</span>
            </div>
            ...
            {featured && (
              ...
              >
                <div className="relative mb-1">
                  ...
                  />
                  <span className="absolute left-1 top-1 rounded-full bg-white px-1 py-0.5 text-[7.5px] font-bold uppercase tracking-wide text-[#10162B]">
                    {t("live.featured", "EN VEDETTE")}
                  </span>
                  {auctionOnFeatured && (
                    <span
                      className="absolute bottom-1 right-1 flex items-center gap-0.5 rounded-full px-1 py-0.5 text-[9px] font-black tabular-nums"
                      style={{
                        background:
                          secondsLeft <= 10
                            ? "oklch(0.82 0.14 85)"
                ...
                {auctionOnFeatured ? (
                  <>
                    <div className="mt-0.5 text-[8px] font-semibold uppercase tracking-wide text-white/65">
                  ...
                  <div className="mt-0.5 text-[10px] font-bold tabular-nums text-white/95">
                ...
                <div
                  className="mt-1 rounded-md py-0.5 text-center text-[9px] font-black uppercase tracking-wide"
                  style={{
        ...
        {/* Row 2: KiDi+ comments — left, max width so FB crop doesn't clip */}
        <div className="mt-2 max-w-[58%] space-y-1.5 self-start">
          <div
          ...
          {chatLines.length === 0 ? (
            <div
              className="rounded-xl px-2.5 py-2 text-[11px] font-semibold text-white/85"
              style={{ background: "rgba(0,0,0,0.45)" }}
            >
      ...
      {/* No FloatingHearts here — Facebook/YouTube already show native likes
          on the right/bottom and our hearts were fighting that column. */}
