// GET /api/paypal-checkout/return
// -------------------------------
// PayPal redirects here after approve/cancel. Capture + mark order paid,
// then hand off via kidiplus://paypal-done?kind=order (native) or web redirect.
...
export const Route = createFileRoute("/api/paypal-checkout/return")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (cancelled) {
          ...
          return nativeDoneHtml({
            deepLink: "kidiplus://paypal-done?kind=order&status=cancelled",
        ...
        if (!token) {
          ...
          return nativeDoneHtml({
            deepLink: "kidiplus://paypal-done?kind=order&status=error",
        ...
        const result = await finalizePaypalOrderPayment(token);
        if (result.ok) {
          const params = {
            status: "ok",
          ...
          const q = new URLSearchParams({ kind: "order", ...params });
          return nativeDoneHtml({
            deepLink: `kidiplus://paypal-done?${q.toString()}`,
            title: "Paiement confirmé",
            body: "Ta commande est payée. Reviens dans KiDi+.",
            tone: "ok",
            logoUrl,
          });
        }
