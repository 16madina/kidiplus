// GET /api/paydunya-topup/return
// -------------------------------
// PayDunya redirects here after payment/cancel (inside SFSafariViewController /
// Chrome Custom Tab). PayDunya appends `?token=<invoiceToken>`. We confirm +
// credit on the server, then hand off to the app — mirroring the PayPal return
// route (never auto-redirect native into the full SPA).
...
function webRedirect(origin: string, params: Record<string, string>): Response {
  ...
  u.searchParams.set("paydunya_done", "1");
...
export const Route = createFileRoute("/api/paydunya-topup/return")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // PayDunya appends `token` to the return URL.
        ...
        if (cancelled) {
          ...
          return nativeDoneHtml({
            deepLink: "kidiplus://paydunya-done?status=cancelled",
        ...
        if (!token) {
          ...
          return nativeDoneHtml({
            deepLink: "kidiplus://paydunya-done?status=error",
        ...
        const result = await finalizePaydunyaTopup(token);
        if (result.ok) {
          const params = {
            status: "ok",
          ...
          return nativeDoneHtml({
            deepLink: `kidiplus://paydunya-done?${q.toString()}`,
        ...
        const params = { status: "pending", reason: result.error.slice(0, 80) };
        if (!preferNative) return webRedirect(origin, params);
        const q = new URLSearchParams(params);
        return nativeDoneHtml({
          deepLink: `kidiplus://paydunya-done?${q.toString()}`,
