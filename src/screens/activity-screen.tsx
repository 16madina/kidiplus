...
import { Bell, Radio, Truck, Trash2, Inbox, Check, PackageCheck, ShieldCheck, Heart, MessageCircle, Swords } from "lucide-react";
...
function NotifKindIcon({ kind }: { kind: string }) {
  ...
  else if (kind === "battle") { icon = <Swords size={18} />; bg = "oklch(0.45 0.12 85)"; }
  else if (kind === "vitrine_like") { icon = <Heart size={18} />; bg = "oklch(0.62 0.2 25)"; }
  else if (kind === "vitrine_comment" || kind === "vitrine_comment_reply" || kind === "vitrine_comment_like") { icon = <MessageCircle size={18} />; bg = "oklch(0.6 0.16 230)"; }
  return (
    <div
