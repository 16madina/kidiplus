...
async function runRegistration(userId: string) {
  ...
  try {
    ...
    // Prefer FCM registration token (required by backend sendFcmToTokens).
    // On iOS, PushNotifications alone returns an APNs device token.
    let token: string | undefined;
    try {
      const fcm = await FirebaseMessaging.getToken();
      token = fcm.token;
    } catch (e) {
      await log(platform, "fcm_get_token_failed", false, e instanceof Error ? e.message : String(e));
    }
    if (!token && platform === "android") {
      // Fallback: Capacitor Push registration event is already an FCM token on Android.
