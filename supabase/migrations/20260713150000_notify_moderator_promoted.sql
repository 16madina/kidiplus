-- Notify a user when they are promoted to live moderator.
-- Inserts into public.notifications → existing trg_notifications_fanout → FCM push.
...
BEGIN
  SELECT
    COALESCE(NULLIF(trim(p.display_name), ''), NULLIF(trim(p.handle), ''), 'Un hôte')
  INTO v_host_name
...
EXCEPTION WHEN OTHERS THEN
  -- Never block the promote insert if notify fails.
  RETURN NEW;
END;
$$;
