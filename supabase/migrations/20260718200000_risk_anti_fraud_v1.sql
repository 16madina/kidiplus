-- Anti-fraud / AML V1: daily limits, payout verification gate, risk alerts.
-- Limits mirrored in src/lib/risk-limits.ts — keep in sync.
...
create or replace function public.risk_daily_cap(
  ...
  _kind text,
  _currency text,
  _account_age_hours numeric default 9999
)
returns numeric
language plpgsql
immutable
...
begin
  ...
  if _kind = 'topup' then
    v_unit_max := case v_cur
      when 'XOF' then 300000
      when 'CAD' then 500
      else 500
    end;
    ...
    -- new: 1× unit max; first 24h → 50% of unit max
    if coalesce(_account_age_hours, 0) < 24 then
      return v_unit_max * 0.5;
    end if;
    return v_unit_max;
  end if;
...
-- Check (+ optionally consume) a daily limit. Returns jsonb {ok, error?, ...}.
...
begin
  ...
  -- Ensure a row exists so FOR UPDATE serializes concurrent checks.
  ...
  if _consume then
    ...
    on conflict (user_id, day, currency) do update set
      topup = public.risk_daily_usage.topup
        + case when _kind = 'topup' then excluded.topup else 0 end,
      spend = public.risk_daily_usage.spend
        + case when _kind = 'spend' then excluded.spend else 0 end,
      gift_received = public.risk_daily_usage.gift_received
        + case when _kind = 'gift_received' then excluded.gift_received else 0 end;
  end if;
...
-- Authenticated helper: check/consume for self only.
...
-- Preflight for Stripe top-up API (service role): moderation + daily check, no consume.
...
-- ---------------------------------------------------------------------------
-- 3) Patch credit_wallet_topup — consume topup limit after successful credit
-- ---------------------------------------------------------------------------
...
begin
  ...
  if coalesce((v_risk->>'ok')::boolean, false) is not true then
    -- Payment succeeded at Stripe but daily limit blocks credit: alert + fail
    -- so ops can reconcile manually (rare race after preflight).
...
begin
  ...
  -- Preflight both limits (no consume), then consume — avoids burning sender spend
  -- if the seller has hit their gift_received cap.
  ...
  v_risk := public.risk_check_and_consume(
    v_live.seller_id, 'gift_received', v_seller_net, v_live_currency, false
  );
  if coalesce((v_risk->>'ok')::boolean, false) is not true then
    return jsonb_build_object(
      'ok', false,
      'error', 'seller_gift_limit',
      'cap', v_risk->'cap',
      'used', v_risk->'used',
  ...
  perform public.risk_check_and_consume(
    v_live.seller_id, 'gift_received', v_seller_net, v_live_currency, true
  );
...
-- ---------------------------------------------------------------------------
-- 6) Patch request_payout — verified gate + XOF min 5000 + velocity alert
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Velocity: top-up then gift income then payout within 2h → alert (still allow)
  select exists (
    select 1 from public.wallet_transactions
...
begin
  ...
  select count(*)::int into v_total
   ...
   where case
           when _status = 'resolved' then a.resolved_at is not null
           when _status = 'all' then true
           else a.resolved_at is null
         end;
...
begin
  ...
  update public.profiles
     set risk_restricted = coalesce(_restricted, false)
   where id = _user_id;
  if not found then
    return jsonb_build_object('ok', false, 'error', 'not_found');
  end if;
  perform public.risk_raise_alert(_user_id, 'admin_risk_restricted', jsonb_build_object(
    'restricted', coalesce(_restricted, false),
    'by', v_user
  ));
