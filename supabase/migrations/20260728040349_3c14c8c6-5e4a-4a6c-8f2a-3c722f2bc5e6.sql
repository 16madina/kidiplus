-- Restore auction wallet auto-pay when the winner has enough Solde KiDi+.
-- If balance is insufficient, order stays pending (24h) for Pay now later.
...
BEGIN
  ...
  -- Wallet auto-pay when the winner has enough balance (incl. cross-currency FX).
  -- Otherwise leave pending so they can top up / pay within 24h from Mes achats.
