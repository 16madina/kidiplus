-- Direct messaging (buyer ↔ seller / user ↔ user).
--
-- Design:
-- - dm_threads: one row per user pair (user_a < user_b enforced) with
--   denormalized last-message metadata for the inbox list.
-- - dm_messages: append-only messages; read_at set by the recipient.
-- - No client INSERT/UPDATE: all writes go through SECURITY DEFINER RPCs
--   that enforce moderation status (assert_user_active) and blocks.
-- - Realtime: clients subscribe to postgres_changes on both tables; RLS
--   restricts rows to thread participants.
...
-- Raise if either user blocked the other.
...
-- Find an existing thread with _other (NULL if none). Read-only.
...
-- Send a DM to _to. Creates the thread if needed. Returns jsonb
-- { thread_id, message: {...} }.
...
-- Inbox: my threads with the other user's profile + unread count.
...
-- Messages of a thread (participant only), newest first.
